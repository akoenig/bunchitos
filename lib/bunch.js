...
/**
 * Removed the prefix from a name.
 *
 * @param  {string} name The name from which the prefix should be removed.
 *
 * @return {string} The name without the prefix.
 *
 */
Bunch.prototype.$$removePrefix = function $$removePrefix (name) {
...
Bunch.prototype.fromNodeModules = function fromNodeModules () {
    ...
    for (i; i < len; i = i + 1) {
        ...
        if (this.$$hasPrefix(name) && fs.existsSync(path.join(source, name, 'package.json'))) {
            components[this.$$camel(this.$$removePrefix(name))] = require(name);
        }
    }
...
Bunch.prototype.fromPackageJson = function fromPackageJson () {
    ...
    for (prop in declarations) {
        if (declarations.hasOwnProperty(prop) && this.$$hasPrefix(prop)) {
            components[this.$$camel(this.$$removePrefix(prop))] = require(prop);
        }
    }
